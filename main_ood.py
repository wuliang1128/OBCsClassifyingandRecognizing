...
def get_args():
    ...
    parser.add_argument("--ood_methods", type=list,
                        default=["max_prob", "max_logit", "energy", "GEN", "shannon_entropy"],
                        help="the ood score function")
    parser.add_argument("--GEN_M", type=int, default=6)
    parser.add_argument("--GEN_gamma", type=float, default=0.1)
...
def main(args):
    ...
    #     print(f"INFO using pretrained model from {args.pretrain_path}")
    #     path = osp.join(args.pretrain_path, f"split_{args.split}_latest.pth")
    ...
    for epoch in range(args.epochs):
        ...
        scheduler.step()
        # compute AUROC
        acc, AUROCs = eval_ood(model, val_loader, out_loader, device, args)
        info = f"acc: {acc * 100:.2f}"
        print(info)
        for ood_method in AUROCs.keys():
            info = f"AUROC {ood_method}: {AUROCs[ood_method] * 100:.2f}"
            print(info)
            save_content.append(info)
            save_content.append('\n')
...
if __name__ == '__main__':
    ...
    mean_acc = []
    mean_auroc = {}
    for i in range(len(args.ood_methods)):
        mean_auroc[args.ood_methods[i]] = []
    for item in range(len(OrcaleMNIST_split)):
        args.split = item
        ...
        mean_acc.append(acc)
        for method in AUROCs.keys():
            mean_auroc[method] = AUROCs[method]
    ...
    # compute the average acc and AUROCs
    mean_info = []
    mean_acc = np.array(mean_acc).mean()
    mean_info.append(f"Mean acc: {mean_acc}\n")
    for method in args.ood_methods:
        mean_auroc[method] = np.array(mean_auroc[method]).mean()
        mean_info.append(f"Mean {method} AUROC: {mean_auroc[method]}\n")
    with open(file_name, 'a+') as file:
        file.writelines(mean_info)
