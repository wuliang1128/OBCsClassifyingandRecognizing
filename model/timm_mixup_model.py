...
def train_net(model, loader, criterion, optimizer, device, args):
    ...
    for inputs, labels, _ in loader:
        ...
        if 'mixup' in args.da_method:
            inputs_new, labels_new = mixup(inputs, labels, args, intra=intra)
        # elif 'cutmix' in args.da_method:
        #     inputs_new, labels_new = CutMix(inputs, labels, intraClass=intra)
        else:
            inputs_new, labels_new = inputs, F.one_hot(labels)
...
def get_pair_index(labels, intra=True):
    new_index = []
    for index, value in enumerate(labels):
        if intra:
            candidates = [i for i in range(len(labels)) if i != index and labels[i] == labels[index]]
        else:
            candidates = [i for i in range(len(labels)) if i != index and labels[i] != labels[index]]
        xj_index = random.choice(candidates) if len(candidates) > 0 else index
        new_index.append(xj_index)
    return new_index


def mixup(inputs, labels, args, intra=True):
    alpha = args.alpha
    new_index = get_pair_index(labels, intra=intra)
...
def Save_ImageTensor_ToImage(image_tensor, label, str_prefix):
    ...
    image_array = np.copy(gray_tensor)  # shape[224,224] min:0.0 max:1.0
